...
// .wrangler/tmp/bundle-6qFqUB/checked-fetch.js
        ...
        if (level) {
          ...
          if (alertRule) {
            const intakeResult = await c.env.DB.prepare(`
              ...
              VALUES (?, ?, ?)
            `).bind(activity_log_id, alertRule.id, level).run();
        ...
        if (level && level !== "\u98F2\u3093\u3067\u306A\u3044" && level !== "\u98F2\u3093\u3067\u3044\u306A\u3044") {
          if (tonyoUsed) {
            const intakeResult = await c.env.DB.prepare(`
              ...
              VALUES (?, NULL, ?, ?)
            `).bind(activity_log_id, level, timeSlot).run();
            if (intakeResult.success) {
            ...
            if (alertRuleResult) {
              const intakeResult = await c.env.DB.prepare(`
                INSERT INTO intake_results (activity_log_id, alert_rule_id, intake_status) 
                VALUES (?, ?, ?)
              `).bind(activity_log_id, alertRuleResult.id, level).run();
              if (intakeResult.success) {
                results.intake_results_created.push({
                  type: "scheduled",
...
var diagnosis_default = diagnosis;

// src/routes/timeslots.ts
async function getTimeslots(c) {
  try {
    const userId = 1;
    ...
    const formattedTimeslots = timeslots.results.map((slot) => ({
      id: slot.alert_time,
...
__name(getTimeslots, "getTimeslots");
...
routes.route("/diagnosis", diagnosis_default);
...
// .wrangler/tmp/bundle-6qFqUB/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-6qFqUB/middleware-loader.entry.ts
